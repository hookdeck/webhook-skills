...
import {
  ...
  KNOWN_EVENT_TYPES,
  POST,
} from '../app/webhooks/cloudinary/route';

const API_SECRET = 'test_api_secret_abcd1234';

beforeAll(() => {
  process.env.CLOUDINARY_API_SECRET = API_SECRET;
  cloudinary.config({ api_secret: API_SECRET, signature_algorithm: 'sha1' });
});
...
// helper: hex digest of <algorithm>(rawBody + timestamp + api_secret).
function sign(rawBody: string, timestamp: number, algorithm = 'sha1'): string {
...
function makeRequest(rawBody: string, headers: Record<string, string> = {}): NextRequest {
  return new NextRequest('http://localhost:3000/webhooks/cloudinary', {
    method: 'POST',
...
describe('verifyCloudinarySignature', () => {
  ...
  it('returns false for a wrong signature', () => {
    expect(verifyCloudinarySignature(payload(), now(), 'deadbeef')).toBe(false);
  });

  it('returns false when the signature is missing', () => {
    expect(verifyCloudinarySignature(payload(), now(), null)).toBe(false);
  });

  it('returns false when the body was tampered with after signing', () => {
    const ts = now();
    const signature = sign(payload(), ts, 'sha1');
    expect(verifyCloudinarySignature(payload({ public_id: 'attacker' }), ts, signature)).toBe(false);
  });
});
...
describe('POST /webhooks/cloudinary', () => {
  ...
  it('returns 401 when signed with the wrong secret', async () => {
    ...
    const res = await POST(
      makeRequest(body, { 'x-cld-timestamp': String(ts), 'x-cld-signature': wrongSig })
    );
    expect(res.status).toBe(401);
  });
  ...
  it('returns 401 when the body is tampered with after signing', async () => {
    ...
    const signature = sign(payload(), ts, 'sha1');
    const res = await POST(
      makeRequest(payload({ public_id: 'attacker' }), {
        'x-cld-timestamp': String(ts),
  ...
  it('returns 200 for a valid sha1 signature', async () => {
    ...
    const body = payload();
    const res = await POST(
      makeRequest(body, { 'x-cld-timestamp': String(ts), 'x-cld-signature': sign(body, ts, 'sha1') })
    );
