# Claude Managed Agents Webhooks - Next.js Example

Next.js App Router example for receiving Anthropic Claude Managed Agents (CMA) webhooks with Standard Webhooks signature verification.
...
3. Add your signing key to `.env.local`:
   ```
...
- `POST /webhooks/claude-managed-agents` — webhook receiver (in `app/webhooks/claude-managed-agents/route.ts`)
