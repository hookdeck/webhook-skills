...
| [Alchemy](https://www.alchemy.com/docs/reference/webhooks-overview) | [`alchemy-webhooks`](skills/alchemy-webhooks/) | Verify Alchemy Notify webhook signatures (`X-Alchemy-Signature`, HMAC-SHA256 hex over the raw body), handle ADDRESS_ACTIVITY, NFT_ACTIVITY, and GRAPHQL events |
...
| [Aircall](https://developer.aircall.io/docs/webhooks-overview) | [`aircall-webhooks`](skills/aircall-webhooks/) | Verify Aircall webhooks by timing-safe comparison of the `token` field in the JSON body (no signature header, no HMAC), handle cloud phone events like call.created, call.answered, call.ended, message.received, contact.updated, and user.connected.v2 |
...
| [Azure Event Grid](https://learn.microsoft.com/en-us/azure/event-grid/end-point-validation-event-grid-events-schema) | [`azure-event-grid-webhooks`](skills/azure-event-grid-webhooks/) | Receive Azure Event Grid deliveries, which carry **no payload signature at all** — trust comes from the ownership handshake (`Microsoft.EventGrid.SubscriptionValidationEvent` echoed as `validationResponse` with HTTP 200, or the CloudEvents v1.0 `OPTIONS` abuse-protection preflight) plus channel auth via a static delivery-property header, a query-parameter client secret, or a Microsoft Entra ID bearer token; guard on `aeg-subscription-name` and parse Event Grid arrays vs CloudEvents objects |
| [BaseLinker](https://api.baselinker.com/) | [`baselinker-webhooks`](skills/baselinker-webhooks/) | Receive BaseLinker (Base.com) order callbacks, which arrive as bodyless HTTP **HEAD** requests with the whole payload in the query string (observed: `order_id`, `state`) — there is **no signature, secret, or handshake** to verify, so respond with a bare bodyless 200 and fetch detail via `getOrders` (`X-BLToken`) |
...
| [Community](https://developer.community.com/reference/webhooks-introduction) | [`community-webhooks`](skills/community-webhooks/) | Verify Community (community.com SMS platform) webhook signatures (`community-signature`, HMAC-SHA256 hex over `{t}.{raw_body}`), handle message.inbound, message.outbound, member.created, member.updated, and member.deleted events |
...
| [Cronofy](https://docs.cronofy.com/developers/api/push-notifications/) | [`cronofy-webhooks`](skills/cronofy-webhooks/) | Verify Cronofy push notifications (`Cronofy-HMAC-SHA256`, a comma-separated list of base64 HMAC-SHA256 digests over the raw body keyed with the app client secret, any match wins), handle verification, change, profile_disconnected, conferencing_profile_disconnected, profile_initial_sync_completed, and gdpr_requested notifications |
...
| [Formstack](https://help.formstack.com/hc/en-us/articles/44592535914387-WebHook-Submit-Actions) | [`formstack-webhooks`](skills/formstack-webhooks/) | Verify Formstack Forms WebHook submit action signatures (`X-FS-Signature`, HMAC-SHA256 lowercase hex over the raw body, optional `sha256=` prefix, user-overridable header name), handle urlencoded and JSON submission payloads, and route on `FormID` — Formstack has no event types, only form submissions |
...
| [Google Cloud Pub/Sub](https://cloud.google.com/pubsub/docs/push) | [`google-pubsub-webhooks`](skills/google-pubsub-webhooks/) | Verify Google Cloud Pub/Sub push subscriptions — there is no signing secret or HMAC header; validate the Google-signed OIDC `Authorization: Bearer` JWT (`iss`, `aud`, `email`, `email_verified`), parse the `message.data` / `attributes` envelope, and ack within the deadline |
...
| [MailerSend](https://developers.mailersend.com/api/v1/account/webhooks.html) | [`mailersend-webhooks`](skills/mailersend-webhooks/) | Verify MailerSend webhooks (`Signature` header, HMAC-SHA256 hex over the raw body), handle the `webhook.test` URL-validation ping and activity.sent, activity.delivered, activity.hard_bounced, activity.opened, activity.clicked, and activity.spam_complaint events |
...
| [Quo](https://support.quo.com/core-concepts/integrations/webhooks) | [`quo-webhooks`](skills/quo-webhooks/) | Verify Quo (formerly OpenPhone) webhooks across BOTH of its signature schemes — the current Standard Webhooks style `webhook-id`/`webhook-timestamp`/`webhook-signature` (HMAC-SHA256 base64 over `{id}.{timestamp}.{raw-body}`, `whsec_` key) and the legacy `openphone-signature` (`hmac;1;<ms-timestamp>;<base64>` over `{timestamp}.{raw-body}`) — branch on the `data.resource` vs `data.object` envelope, and handle `message.received`, `call.completed`, `call.summary.completed`, `contact.updated` and `task.*` events. Not Quoter |
...
| [SendGrid Inbound Parse](https://www.twilio.com/docs/sendgrid/for-developers/parsing-email/setting-up-the-inbound-parse-webhook) | [`sendgrid-inbound-webhooks`](skills/sendgrid-inbound-webhooks/) | Receive inbound email from SendGrid Inbound Parse (multipart/form-data), verify the opt-in ECDSA P-256 signature (`X-Twilio-Email-Event-Webhook-Signature`) over the raw body, handle the OAuth Bearer path, parse both the default and raw (`send_raw`) formats, and route on `envelope.to` |
